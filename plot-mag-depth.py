...
##############################################################################  
print("3. Same as above but combining Field 3 and 4.")
r_exp = np.concatenate((r_exp3, r_exp4))
gf, rf, zf = combine_grz(grzflux3, grzflux4)
gi, ri, zi = combine_grz(grzivar3, grzivar4)
gf_err, rf_err, zf_err = grz_flux_error([gi, ri, zi])
...
plt.title("Field 34", fontsize=ft_size)
plt.tick_params(axis='both', which='major', labelsize=tick_size)    
plt.savefig("mag-depth-field34.png",bbox_inches="tight",dpi=400)
# plt.show()
plt.close()
print("Completed.\n")
