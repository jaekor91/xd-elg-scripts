...
##############################################################################
print("1. Import original targets working")
...
print("16hr reobs targets")
fits = load_fits_table(MMT_data_dir+"MMTtargets-DEEP2-16hrs-reobs.fits")
ra16_reobs, dec16_reobs = load_radec(fits)
grz16_reobs = load_grz(fits)
iColor = MMT_study_color(grz16_reobs, 0)
iDECaLS_quality = MMT_DECaLS_quality(fits)
print("# of targets: %d" % iColor.size)
print("# color violation: ",-(iColor.sum()-iColor.size))
print("# decals quality violation", -(iDECaLS_quality.sum()-iDECaLS_quality.size))
...
print("23hr reobs targets")
fits = load_fits_table(MMT_data_dir+"MMTtargets-DEEP2-23hrs-reobs.fits")
ra23_reobs, dec23_reobs = load_radec(fits)
grz23_reobs = load_grz(fits)
iColor = MMT_study_color(grz23_reobs, 1)
iDECaLS_quality = MMT_DECaLS_quality(fits)
print("# of targets: %d" % iColor.size)
print("# color violation: ",-(iColor.sum()-iColor.size))
print("# decals quality violation", -(iDECaLS_quality.sum()-iDECaLS_quality.size))
...
print("Note: Here I did not check whether an object was DEEP2 color selected/rejected \
nor whether stellar mask was properly applied (as previously defined).")
